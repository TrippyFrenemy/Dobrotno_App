        ...
        while current <= end:
            ...
            if r:
                ...
                for e in store_emps + wh_emps:
                    amount = Decimal(e.salary) if e.salary is not None else Decimal(e.user.default_rate)
                    salary_acc[e.user_id] = salary_acc.get(e.user_id, Decimal("0.00")) + amount
                days.append(
                    {
                        "id": r.id,
                        "date": r.date,
...
@router.get("/{store_id}/records/{record_id}", response_class=JSONResponse)
async def view_record(
    ...
    return {
        ...
        "cash_on_hand": float(record.cash_on_hand or 0),
        "terminal": float(record.terminal or 0),
        "changed_price": float(record.changed_price or 0),
        "cash_total": float(record.cash_total or 0),
        "discount": float(record.discount or 0),
        "promotion": float(record.promotion or 0),
        "to_store": float(record.to_store or 0),
        "refund": float(record.refund or 0),
        "service": float(record.service or 0),
        "receipt": float(record.receipt or 0),
        "expenses": float(record.expenses or 0),
        "salary_expenses": float(record.salary_expenses or 0),
    }
...
async def edit_record_page(
    ...
    start_times = {
        e.user_id: (e.start_time or e.user.shift_start).strftime("%H:%M")
        for e in record.employees
    }
    end_times = {
        e.user_id: (e.end_time or e.user.shift_end).strftime("%H:%M")
        for e in record.employees
    }
    vac_q = await session.execute(
...
async def edit_record(
    ...
    if not csrf_token or not await verify_csrf_token(user.id, csrf_token):
        raise HTTPException(status_code=403, detail="Invalid CSRF token")
    record = await session.get(
        StoreShiftRecord, record_id, options=[selectinload(StoreShiftRecord.employees)]
    )
    if not record or record.store_id != store_id:
        raise HTTPException(status_code=404, detail="Смена не найдена")
    if not store_employees:
        raise HTTPException(
            status_code=400,
            detail="Нужно выбрать хотя бы одного сотрудника магазина",
        )
    record.date = date_
    record.cash = cash
